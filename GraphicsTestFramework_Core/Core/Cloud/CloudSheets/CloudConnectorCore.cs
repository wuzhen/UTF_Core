...
public static class CloudConnectorCore
{
	...
	public enum QueryType
	{
		...
		createObjects,		// Adds multiple rows to a existing table type with the specified fields.
	...
	public const string MSG_OBJS_CREATED_OK = "OBJS_CREATED_OK";
	public const string MSG_TBL_CREATED_OK = "TBL_CREATED_OK";
	public const string MSG_TBL_EXISTS = "TBL_EXISTS";
	public const string MSG_OBJ_DATA = "OBJ_DATA";
	public const string MSG_BASELINE_DATA = "BASELINE_DATA";
	...
	public const string MSG_OBJ_REPL = "OBJ_RPL_OK";
	...
	/// <summary>
	/// Will create new objects (new rows in a worksheet) with the specified json data array. 
	/// Expects a json string array with the object, as well as the type/table name. 
	/// If not all type fields available in the table are passed, those missing will be filled as "null".
	/// Fields passed but not in the table, will be ignored.
	/// </summary>
	/// <param name="jsonObjectArray">Json string array with the object to be persisted.</param>
	/// <param name="objTypeName">Name of the table that will hold the object.</param>
	/// <param name="runtime">Bool value indicating if the request was sent from Unity Editor or running game.</param>
	public static void CreateObjects(string[] jsonObject, string objTypeName, bool runtime = true)
	{
		isWaiting = true;
		Dictionary<string, string> form = new Dictionary<string, string>();
		...
		//form.Add("isJson", "true");
		form.Add("type", objTypeName);
		form.Add("num", jsonObject.Length.ToString());

		for (int i = 0; i < jsonObject.Length; i++)
		{
			form.Add("field" + i.ToString(), jsonObject[i]);
		}
	...
	/// <summary>
	/// Will check to seee if a table exists (worksheet on the spreadsheet) with the specified name. 
	/// Expects the type/table name to be used.
	/// </summary>
	/// <param name="fields">String array with the names of the table headers.</param>
	/// <param name="tableTypeName">The name of the table to be created.</param>
	/// <param name="runtime">Bool value indicating if the request was sent from Unity Editor or running game.</param>
	public static void TableExists(string tableTypeName, bool runtime = true)
	{
		isWaiting = true;
		Dictionary<string, string> form = new Dictionary<string, string>();
		form.Add("action", QueryType.tableExists.ToString());
		form.Add ("type", tableTypeName);
	...
	/// <summary>
	/// Replaces a row in the sheet based on the fields sent
	/// </summary>
	/// <param name="objTypeName">Name of the table to search.</param>
	...
	/// <summary>
	/// Fetches all the baseline data for the suites loaded, for this platform/API
	/// </summary>
	/// <param name="objTypeNames">Array of sheet names(suites) to pull from</param>
	/// <param name="platform">Platform to pull.</param>
	/// <param name="api">API currently running to pul.</param>
	/// <param name="runtime">Bool value indicating if the request was sent from Unity Editor or running game.</param>
	public static void GetBaselineData( string[] objTypeNames,
		string platform,
		string api,
		bool runtime = true )
	{
		isWaiting = true;
		Dictionary<string, string> form = new Dictionary<string, string>();
		form.Add("action", QueryType.getBaselineData.ToString());
		form.Add ("num", objTypeNames.Length.ToString ());
		form.Add("platform", platform);
		form.Add("api", api);

		for (int i = 0; i < objTypeNames.Length; i++)
		{
			form.Add("type" + i.ToString(), objTypeNames[i]);
		}
		SendRequest(form, runtime);
	}
	...
	public static void UnpackJson(string response)
	{
		...
		// Response for GetBaselineData()
		if (response.StartsWith(MSG_BASELINE_DATA)) 
		{
			parsed = response.Substring(MSG_BASELINE_DATA.Length + 1);
			...
			returnType = QueryType.getBaselineData;
		}
		...
		// Response for TableExists()
		if (response.StartsWith(MSG_TBL_EXISTS))
		{
			parsed = response.Substring(MSG_TBL_EXISTS.Length + 1);
			objTypeName.Add(parsed.Substring(0, parsed.IndexOf(TYPE_END)));
			jsonData.Add(parsed.Substring(parsed.IndexOf(TYPE_END) + TYPE_END.Length));
			returnType = QueryType.tableExists;
		}
	...
	public static void ProcessResponse(string response, float time)
	{
		...
		if (response.StartsWith(MSG_BASELINE_DATA))// TODO - getting baseline data back
		{
			UnpackJson (response);
			response = MSG_BASELINE_DATA;
			unpacked = true;
		}
		...
		case MSG_OBJS_CREATED_OK:
			logOutput ="Objects saved correctly.";
			break;
		...
		case MSG_TBL_EXISTS:
			logOutput = "Table check complete.";
			break;
		...
		case MSG_OBJ_REPL:
			logOutput = "Object replaced correctly.";
			break;
